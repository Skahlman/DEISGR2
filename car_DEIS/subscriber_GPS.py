...
class MinimalSubscriber(Node):
    ...
    def move_2_point(self,goal, coords, current_h):
        if current_h == 1337:
            ...
            if rel_to_goal[0] < 40 and len(self.path) == 1:
                speeds = [200,200]
                self.at_point = True
            else:
                if rel_to_goal[0] < 200 and len(self.path) > 1:
                    self.at_point = True
                speeds = [55,55]
                #print("Going forward")
                v = 4*rel_to_goal[0]
        ...
        if speed > 100:
            ...
            data_2_send.append(int(speed/100))
            data_2_send.append(0)
            data_2_send.append(int(speed/100))
        else:
            for s in speeds:
    ...
    def listener_callback(self, msg):
        ...
        if self.go_home and our_coords[0][0] > 0:
            self.path = path_finding(our_coords[0])
            self.path = self.path[10:-1:6]
            self.go_home = False
        ...
        """
        if our_coords[0][0] > 0 and our_coords[1][0] > 0 and self.path is not None and len(self.path) > 0:
